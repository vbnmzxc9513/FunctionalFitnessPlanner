...
import { CheckCircle2, Circle, Activity, Flame, Shield, Zap, RefreshCw, Info, CalendarDays, Dumbbell, BarChart3, LogIn, LogOut, Brain, Loader2, Settings, Key, ExternalLink } from 'lucide-react';
...
// --- Gemini AI Function ---
const generateAIPlan = async (lastWeekData, currentLevel, lastWeekFeedback, userApiKey) => {
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState('train'); // train | history | settings
  ...
  const requestAIPlan = async () => {
    ...
    try {
      ...
      const aiResponse = await generateAIPlan(lastWeekData, difficultyLevel, lastWeekFeedback, apiKey);
  ...
  if (authMode === 'pending') return <div className="min-h-screen bg-slate-900 flex items-center justify-center text-white"><Loader2 className="animate-spin mr-2"/>載入中...</div>;
  if (authMode === 'login') {
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 font-sans">
        <div className="bg-slate-800 p-8 rounded-3xl shadow-2xl max-w-sm w-full text-center border border-slate-700">
          ...
          <p className="text-slate-400 text-sm mb-8">綁定 Google 帳號，並攜帶你專屬的 API 金鑰，讓 AI 追蹤你的訓練數據。</p>
          <button 
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-200 font-sans pb-24 relative">
      {/* Toast Notification */}
      {toastMsg && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 animate-in fade-in slide-in-from-top-4">
          <div className="bg-slate-800 border border-slate-600 shadow-xl rounded-full px-6 py-3 flex items-center">
      ...
      <header className="bg-slate-800 p-6 rounded-b-3xl shadow-lg border-b border-slate-700 relative">
        ...
        <div className="max-w-md mx-auto">
          ...
          {activeTab === 'history' && <div className="pt-2"><p className="text-slate-400 text-sm">持之以恆是功能性訓練的基石。檢視每個月的累積成果。</p></div>}
          {activeTab === 'settings' && <div className="pt-2"><p className="text-slate-400 text-sm">系統設定與 BYOK 金鑰管理。</p></div>}
        </div>
      </header>
        ...
        {activeTab === 'train' && (
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className={`mb-6 p-5 rounded-2xl border ${weeklyPlan ? 'bg-indigo-900/30 border-indigo-500/30' : 'bg-slate-800 border-slate-700'}`}>
              ...
              {!apiKey && !weeklyPlan && (
                 <div className="bg-amber-900/30 border border-amber-700/50 rounded-lg p-3 mb-3 flex items-start">
                   <Key size={16} className="text-amber-500 mr-2 flex-shrink-0 mt-0.5" />
                   <p className="text-xs text-amber-200">偵測到尚未設定 API 金鑰，請先至「設定」綁定金鑰以解鎖 AI 排表功能。</p>
                 </div>
              )}
        ...
        {activeTab === 'settings' && (
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 space-y-6">
            <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700">
              <h2 className="text-lg font-bold text-white mb-4 flex items-center">
                <Key className="mr-2 text-blue-400" size={20} /> AI 教練金鑰綁定 (BYOK)
              </h2>
              <p className="text-sm text-slate-400 mb-4 leading-relaxed">
                為了保障你的雲端資源安全，本系統採用 Bring Your Own Key 模式。請輸入你個人的 Gemini API Key，該金鑰將<strong className="text-emerald-400">僅儲存於你當前的瀏覽器中 (LocalStorage)</strong>，不會上傳至任何雲端資料庫。
              </p>
              ...
              <div className="mb-4">
                ...
                <input 
                  ...
                  placeholder="AIzaSy..." 
                  className="w-full bg-slate-900 border border-slate-600 rounded-xl px-4 py-3 text-slate-200 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all font-mono text-sm"
                />
              </div>
              ...
              {apiKey && (
                <div className="mt-4 p-3 bg-emerald-900/20 border border-emerald-800/50 rounded-lg flex items-start">
                  <CheckCircle2 size={16} className="text-emerald-500 mr-2 flex-shrink-0 mt-0.5" />
                  <p className="text-xs text-emerald-400">本機已成功綁定金鑰，你可以隨時前往「今日課表」啟動 AI 排表功能。</p>
                </div>
              )}
            </div>
          </div>
      ...
      <nav className="fixed bottom-0 w-full max-w-md left-1/2 -translate-x-1/2 bg-slate-800 border-t border-slate-700 px-6 py-3 flex justify-around items-center z-50">
        ...
        <button onClick={() => setActiveTab('settings')} className={`flex flex-col items-center p-2 transition-colors ${activeTab === 'settings' ? 'text-blue-400' : 'text-slate-500 hover:text-slate-300'}`}>
          <Settings size={24} className="mb-1" /> <span className="text-[10px] font-medium">系統設定</span>
        </button>
      </nav>
    </div>
  );
