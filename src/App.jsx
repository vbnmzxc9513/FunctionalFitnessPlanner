...
const generateAIPlan = async (lastWeekData, currentLevel, lastWeekFeedback, userApiKey, allProgress, metricsHistory, userGoal, dailyTime, userMessage, availableExercises, lang = 'zh', selectedModel = 'gemini-2.5-flash') => {
  ...
  1. 給予一段客製化的教練總結建議。你「必須」在這段建議中包含以下三點：
     - (a) 解讀「歷史身體數值趨勢」與「完成度」：點出近期特定肢體肌肉量、體重或體脂的具體變化趨勢，或者是上週的課表完成率。
     - (b) 推斷原因與「給予具體讚美」：根據你看到的走勢或完成度，以專業教練角度解釋原因，而且【請不要吝嗇你的讚美】，如果發現學員有進步（包含但不限於：肌肉量上升、體脂下降、歷史完成度高、或是積極改變難度），請給予強烈、熱情且正向的鼓勵，這對維持學員的運動黏著度非常重要！
     - (c) 課表對策與說明：針對上述觀察到的身體走勢，以及使用者的「近期訓練目標」與「歷史運動紀錄」，說明你接下來要為他排什麼課表？以及「為什麼這些動作與安排能解決上述問題並達到他的目標」。
