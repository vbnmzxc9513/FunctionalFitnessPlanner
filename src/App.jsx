...
const generateAIPlan = async (lastWeekData, currentLevel, lastWeekFeedback, userApiKey, allProgress, metricsHistory, userGoal, dailyTime, userMessage, availableExercises, lang = 'zh', selectedModel = 'gemini-2.5-flash') => {
  ...
  ${Object.entries(availableExercises).map(([k, v]) => `${k}: ${v.name} (${v.type})`).join('\n')}
   ...
   1. 給予一段客製化的教練總結建議。你「必須」在這段建議中包含以下三點，並且【強制依賴 Markdown 語法進行排版】，包含使用 **粗體標示重點**、適當的段落換行、以及條列式項目，確保整段文字易於閱讀且層次分明：
      - **(a) 專業診斷與狀態評估**：解讀「歷史身體數值趨勢」與「完成度」，具體點出體重、體脂或特定肢體肌肉量的變化。除了數據解讀外，請客觀且「專業地」評估學員目前的身體素質狀態（例如：核心力量極佳、上肢力量普通、完成度略顯不足等），讓學員清楚知道自己目前處在什麼程度。
      - **(b) 推斷原因與具體讚美**：根據你看到的走勢或完成度，以專業教練角度解釋背後可能的原因。在維持教練專業度與權威感的前提下，【請不要吝嗇你的讚美】。如果發現學員有進步（包含但不限於：肌肉量上升、體脂下降、歷史完成度高），請給予強烈、熱情且正向的鼓勵，這對維持學員的運動黏著度非常重要！但請避免純粹的「雞湯式」安撫，讚美必須建立在「具體的數據或行為」之上。
      - **(c) 課表對策與目標對焦**：針對上述觀察到的身體狀態與問題，以及使用者的「近期訓練目標」，「清楚且具體地」說明你接下來要為他排什麼課表？必須明確點出「這套課表背後訓練的目的是什麼」，以及「為什麼這些動作能解決上述問題並幫助他達成近期目標」。
