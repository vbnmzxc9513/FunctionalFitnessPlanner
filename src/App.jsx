...
const generateAIPlan = async (lastWeekData, currentLevel, lastWeekFeedback, userApiKey, allProgress, metricsHistory, userGoal, dailyTime, userMessage, availableExercises, lang = 'zh', selectedModel = 'gemini-2.5-flash') => {
  ...
  ${Object.entries(availableExercises).map(([k, v]) => `${k}: ${v.name} (${v.type})`).join('\n')}
   ...
   1. 給予一段客製化的教練總結建議。你「必須」在這段建議中包含以下三點，並且【強制依賴 Markdown 語法進行排版】（使用 **粗體標示重點**、適當的段落換行），確保易於閱讀。切記：這是一封教練寫給學員的對話，請讓文章自然流暢，【絕對不要】在回覆中印出 "(a)", "(b)", "(c)" 或是「具體讚美」、「專業診斷」等這些指示標題字眼，請將這些要素無痕地融入你的文字中：
      - 專業診斷與狀態評估：解讀「歷史身體數值趨勢」與「完成度」，具體點出體重、體脂或特定肢體肌肉量的變化。除了數據解讀外，請客觀且「專業地」評估學員目前的身體素質狀態（例如：核心力量極佳、上肢力量普通、完成度略顯不足等），讓學員清楚知道自己目前處在什麼程度。
      - 推斷原因與讚美引導：根據你看到的走勢或完成度，以專業教練角度解釋背後可能的原因。在維持教練專業度與權威感的前提下，【請不要吝嗇你的讚美】。如果發現學員有進步（包含但不限於：肌肉量上升、體脂下降、歷史完成度高），請給予強烈、熱情且正向的鼓勵，這對維持學員的運動黏著度非常重要！但必須建立在具體的數據或行為之上，且語氣要自然，不要讓人覺得是刻意為之。
      - 課表對策與目標對焦：針對上述觀察到的身體狀態與問題，以及使用者的「近期訓練目標」，「清楚且具體地」說明你接下來要為他排什麼課表？必須明確點出「這套課表背後訓練的目的是什麼」，以及「為什麼這些動作能解決上述問題並幫助他達成近期目標」。
  ...
  if (authMode === 'login') {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 font-sans">
        <div className="bg-white p-8 rounded-3xl shadow-2xl max-w-sm w-full text-center border border-sky-100 relative overflow-hidden">
          {/* 羽球場地線條意象 */}
          <div className="absolute top-0 right-0 w-32 h-32 border-l-2 border-b-2 border-sky-300/10 rounded-bl-xl pointer-events-none"></div>
          <div className="absolute bottom-0 left-0 w-24 h-24 border-t-2 border-r-2 border-sky-300/10 rounded-tr-xl pointer-events-none"></div>
